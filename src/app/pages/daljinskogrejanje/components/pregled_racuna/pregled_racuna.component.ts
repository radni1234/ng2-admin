...
import {RnStavke, MesecLista, RnTip} from "../../../javniobjekti/components/racuni/racundata";
...
import {RacunKotlarnica} from "./pregled_racuna.data";


@Component({
  selector: 'isem-pregled-racuna-kotlarnica',
  templateUrl: 'pregled_racuna.component.html',
  styleUrls: ['../../styles/table.component.scss']
})
export class PregledRacunaKotlarnicaComponent implements OnInit {

  @Input() kotlarnicaId: number;
  ...
  ngOnInit() {
    this.getBrojila(this.kotlarnicaId);
  }
  ...
  getDataRacuni(kotlarnicaId: number) {
    this.crudService.getData("rn_prikaz?kot_id="+kotlarnicaId).subscribe(
  ...
  getBrojila(kotlarnicaId: number) {
    this.crudService.getData("brojilo_kotlarnica/sve?kotlarnica_id="+kotlarnicaId).subscribe(
      data => {
        this.brojila = data;
  ...
  getBrojiloVrstaKolone(brojiloVrstaId: number) {
    this.crudService.getData("bro_vrs_kol/sve?bro_vrs_id="+brojiloVrstaId).subscribe(
  ...
  onDeleteConfirmRacuni() {
    this.crudService.delete("rn_kotlarnica", this.brisanjeRnId)
      .subscribe(
        data => {console.log(data); this.getDataRacuni(this.brojilo.id);},
        error => {console.log(error);}
      );
  ...
  rn: RacunKotlarnica = new RacunKotlarnica();
  ...
  formirajRn(id:number){
    this.proveraRn = 0;
    this.rn = new RacunKotlarnica();

    this.crudService.getSingle("rn_kotlarnica/jedan?id=" + id)
      .subscribe(
        data => {
          this.rn = data;
          ...
          if (this.rn.brojiloKotlarnica.rezimMerenja.id == 3){
  ...
  onCreateNoviRn(){
    ...
    this.rn = new RacunKotlarnica();
    this.nazivKolone = new Array<String>();
    this.rn.brojiloKotlarnica = this.brojilo;
  ...
  onSubmitRn(event) {
    ...
    if (this.myFormRn2.invalid) {
      ...
      console.log('racun');
      console.log(this.rn);
      this.crudService.sendData("rn_kotlarnica", this.rn)
        .subscribe(
          data => {console.log(data); this.getDataRacuni(this.brojilo.id);},
          error => {console.log(error);
          // this.router.navigate(['/login']);
  ...
  public onGodinaSelected(selectedGodina: number){
    ...
    if (this.noviRn || (!this.noviRn && this.rn && (this.godina != this.staraGodina || this.mesec.id != this.stariMesec))) {
      ...
      this.proveriRacun("rn_kotlarnica/provera?datumr="+this.datePipe.transform(this.datumRacuna, 'dd.MM.yyyy')+"&kotlarnica_id="+this.rn.brojiloKotlarnica.id);
    }
