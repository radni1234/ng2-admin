...
let template = require('./selection_tool_nac_fin.component.html');

@Component({
  selector: 'selection-tool-nac-fin',
  template: template
})
export class SelectionToolNacFin implements OnInit{
  ...
  private isGrupeLoaded:boolean = false;
  private isPodgrupeLoaded:boolean = false;
  private isNacFinLoaded:boolean = false;
  private isObjekatLoaded: boolean = false;
  private opstine: any[];
  private mesta: any[];
  errorMessage:string;
  ...
  getObjekte() {
    this.crudService.getData("objekat/lov?ops_id="+this.opstinaID+"&mes_id="+this.mestoID+"&gru_id="+this.grupaID+"&podgru_id="+this.podgrupaID+"&nac_fin_id="+this.nacFinId).subscribe(
      data => {
        this.objekti = data;
        console.log(data);
  ...
  getObjekteSve() {
    this.crudService.getData("objekat/lov?ops_id=0&mes_id=0&gru_id=0&podgru_id=0&nac_fin_id=0").subscribe(
      data => {
        this.objSvi = data;
      },
      error => console.log(error)
  ...
  public onNacFinSelected(selectedId: number) {
    console.log("ID selektovanog nac fin je: " + selectedId);
    this.getObjekte();
  }
  ...
  napuniNacFin (){
    this.crudService.getData("nac_fin/sve")
      .subscribe(
        data => {
          this.nacFin = data;

          console.log(this.nacFin);
          this.isNacFinLoaded = true;
        },
        error => this.errorMessage = <any>error);
