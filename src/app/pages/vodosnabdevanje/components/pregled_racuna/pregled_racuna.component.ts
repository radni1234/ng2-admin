...
import {RacunVodozahvat} from "./pregled_racuna.data";



@Component({
  selector: 'isem-pregled-racuna-vodozahvat',
  templateUrl: 'pregled_racuna.component.html',
  styleUrls: ['../../styles/table.component.scss']
})
export class PregledRacunaVodozahvatComponent implements OnInit {

  @Input() vodozahvatId: number;
  ...
  ngOnInit() {
    this.getBrojila(this.vodozahvatId);
  }
  ...
  getDataRacuni(vodozahvatId: number) {
    this.crudService.getData("rn_prikaz?vod_id="+vodozahvatId).subscribe(
  ...
  getBrojila(vodozahvatId: number) {
    this.crudService.getData("brojilo_vodozahvat/sve?vodozahvat_id="+vodozahvatId).subscribe(
      data => {
        this.brojila = data;
  ...
  onDeleteConfirmRacuni() {
    this.crudService.delete("rn_vodozahvat", this.brisanjeRnId)
      .subscribe(
        data => {console.log(data); this.getDataRacuni(this.brojilo.id);},
        error => {console.log(error);}
      );
  ...
  rn: RacunVodozahvat = new RacunVodozahvat();
  ...
  formirajRn(id:number){
    this.proveraRn = 0;
    this.rn = new RacunVodozahvat();

    this.crudService.getSingle("rn_vodozahvat/jedan?id=" + id)
      .subscribe(
        data => {
          this.rn = data;
          ...
          if (this.rn.brojiloVodozahvat.rezimMerenja.id == 3){
  ...
  onCreateNoviRn(){
    ...
    this.rn = new RacunVodozahvat();
    this.nazivKolone = new Array<String>();
    this.rn.brojiloVodozahvat = this.brojilo;
  ...
  onSubmitRn(event) {
    ...
    if (this.myFormRn2.invalid) {
      ...
      console.log('racun');
      console.log(this.rn);
      this.crudService.sendData("rn_vodozahvat", this.rn)
        .subscribe(
          data => {console.log(data); this.getDataRacuni(this.brojilo.id);},
          error => {console.log(error);
          // this.router.navigate(['/login']);
  ...
  public onGodinaSelected(selectedGodina: number){
    ...
    if (this.noviRn || (!this.noviRn && this.rn && (this.godina != this.staraGodina || this.mesec.id != this.stariMesec))) {
      ...
      this.proveriRacun("rn_vodozahvat/provera?datumr="+this.datePipe.transform(this.datumRacuna, 'dd.MM.yyyy')+"&vodozahvat_id="+this.rn.brojiloVodozahvat.id);
    }
